...
def kelvin_helmholtz(x: np.ndarray, F, Mr, pr=2.5, rhor=1., xr=1., primitives=False, amp=1e-2, ref=False, mode=1) -> np.ndarray:
    primitive = np.empty((*x.shape[:-1], 4))
    X = x[..., 0]
    Y = x[..., 1]
    ...
    primitive[..., 2] = amp * np.sin(2 * np.pi * X * mode)
    if ref:
        primitive[..., 1:3] *= ur
